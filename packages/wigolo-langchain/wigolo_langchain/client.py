...
class WigoloMcpClient:
    ...
    requests over stdin, and reads responses from stdout. Stderr is consumed
    and logged.
    """
    ...
    async def _send_request(self, method: str, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if "error" in response and response["error"] is not None:
            ...
            raise WigoloClientError(
                f"MCP error {err.get('code', 'unknown')}: {err.get('message', 'unknown error')}"
            )
    ...
    async def _read_response(self) -> dict[str, Any]:
        ...
        while True:
            ...
            try:
                ...
                logger.debug("non-JSON stdout line: %s", text[:200])
                continue
